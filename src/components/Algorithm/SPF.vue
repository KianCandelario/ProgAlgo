<template>
  <div class="content" id="app">
    <div class="flex flex-col w-1/4 relative justify-center gap-5 shadow-2xl shadow-black-500/50 rounded-xl h-[50vh] p-5">
      ...
      <form @submit.prevent="calculateSJF">
    ...
    <div class="bg-white flex flex-col w-full relative rounded-[20px] shadow-2xl shadow-black-500/50 p-10" v-if="results.length === 0">
      <h1 class="text-[30px] font-bold text-[#023e8a] font-poppins">Shortest Job First Scheduling Algorithm (SJF)</h1>
      <p class="text-[20px] p-5 font-quicksand font-semibold">Shortest Job First scheduling works on the process with the shortest burst time first.</p>
    ...
    <div class="flex flex-col w-3/4 relative justify-center gap-5 rounded-[20px] bg-white p-10 shadow-2xl shadow-black-500/50" v-if="results.length > 0">
      <div class="relative flex flex-row justify-between items-center">
        ...
        <h2 class="font-poppins border-[#023e8a] border-2 rounded-md p-2 text-[#023e8a] font-bold">SJF</h2>
      </div>
  ...
  methods: {
    calculateSJF() {
      ...
      this.processes = arrivalEntries.map((arrivalTime, index) => ({
        id: String.fromCharCode(65 + index), // Generating alphabetical process IDs without 'P'
        burstTime: burstEntries[index],
        arrivalTime: arrivalTime,
      }));

      this.processes.sort((a, b) => a.arrivalTime - b.arrivalTime); // Sorting processes by arrival time

      const numberOfProcesses = this.processes.length;
      this.numberOfProcesses = numberOfProcesses;

      const { waitingTime, turnaroundTime, ganttBars } = this.calculateSJFDetails();
    ...
    calculateSJFDetails() {
      const numberOfProcesses = this.numberOfProcesses;
      const processes = this.processes.map(p => ({ ...p }));
      ...
      let currentTime = 0;
      let completed = 0;
      const isCompleted = Array(numberOfProcesses).fill(false);
      ...
      while (completed !== numberOfProcesses) {
        ...
        waitingTime[shortestProcessIndex] = currentTime - process.arrivalTime;
        turnaroundTime[shortestProcessIndex] = waitingTime[shortestProcessIndex] + process.burstTime;
